...
g = np.hstack([np.ones(9), 2 * np.ones(6)])  # g为已知样本数据的类别标号    水平堆叠矩阵

# 马氏距离分类
v = np.cov(x0.T)  # 计算协方差
knn = KNeighborsClassifier(2, metric='mahalanobis', metric_params={'V': v})     # 2类、马氏距离
knn.fit(x0, g)
pre = knn.predict(x)
print("马氏距离分类结果：", pre)
...
# 欧氏距离分类
knn2 = KNeighborsClassifier(2)  # 默认欧氏距离
knn2.fit(x0, g)
pre2 = knn2.predict(x)
print("欧氏距离分类结果：", pre2)
